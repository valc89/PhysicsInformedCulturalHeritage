...
class PoissonPOD(object):
    ...
    def __init__(self, mu_range : List[tuple], fem_p : PoissonFEM):
    ...
    def plot_samples(self, figsize = (12, 6)):
        if not self._training_set is None:
            if not self._testing_set is None:
                if self.total_parameters != 1:
                    self._plot_both(figsize)
                else:
                    self._plot_singlepar_trtst(figsize)
            else:
                if self.total_parameters != 1:
                    self._plot_single(figsize)
                else:
                    self._single_plot(figsize)
        else:
    ...
    def solve_rom(self, mu, N):
        u_D = self.fem_p._boundary_cond
        bc = fe.DirichletBC(self.femp_p.V, u_D, fe.DomainBoundary())
        bcs = []
        bcs.append(bc)
        ...
        return self.Z[:N]*reduced_solution, np.array(self.Z[:N]*reduced_solution.vector().get_local())
    ...
    def _plot_both(self, fig_size = (12, 12)):
    ...
    def _plot_single(self, fig_size = (6, 15)):
        n = self.total_parameters
        x = np.arange(self.training_set.shape[0])
    ...
    def _plot_singlepar_trtst(self, fig_size = (11, 6)):
        x = np.arange(self.training_set.shape[0])
        x_t = np.arange(self.testing_set.shape[0])
        count = 0
    ...
    def _single_plot(self, fig_size = (7, 5)):
        count = 0
        x = np.arange(self.training_set.shape[0])
