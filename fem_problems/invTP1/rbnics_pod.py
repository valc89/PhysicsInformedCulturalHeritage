...
class PODReduction(object):

    def __init__(self, mu_range : List[tuple], fem_p):
        self.mu_range = mu_range
        self.fem_p = fem_p
        self.total_parameters = len(mu_range)
        self.pod = rb.backends.dolfin.ProperOrthogonalDecomposition(fem_p.V, fem_p.inner_product)
        self._Z = None
    ...
    def pod_execution(self, N_max, tol):
        self._store_samples()
        self._pod_compression(N_max, tol)
    ...
    def error_analysis(self):
        ...
        for N_basis in range(1, self.num_basis+1):
            ...
            for j in range(self.testing_set.shape[0]):
                ...
                error["abs"][N_basis].append(assl)
                error["rel"][N_basis].append(rel)
    ...
    def _pod_compression(self, N_max, tol):
        self._Nmax, self._tol = N_max, tol
        print("POD COMPRESSION EXECUTION...")
        ...
        print("...End of POD COMPRESSION EXECUTION")

    def _store_samples(self):
        print("STORING SNAPSHOTS...")
        for i in range(self.training_set.shape[0]):
            print(f'+++ Compute and store snaphot {i+1} for mu = {self.training_set[i]} +++')
            _, solution = self.fem_p.solve_fem(self.training_set[i])
            self.pod.store_snapshot(solution)
